...
const DEFAULT_NAV = [
  ...
  { id: 'portfolio', href: '/portfolio', label: 'Cluster', icon: 'Layers', tooltip: 'Combined overview across all car hire sites' },
  ...
  { id: 'internal-links', href: '/internal-links', label: 'Internal Links', icon: 'Link2', tooltip: 'GSC + GA4 + codebase crawl: orphan-fix list with anchor recommendations' },
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-[#1a1d27] border-r border-[#2a2d3a] z-40 flex flex-col transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="p-6 border-b border-[#2a2d3a]">
          ...
          <div className="flex items-center justify-between mt-1">
            <p className="text-xs text-zinc-500">Car Hire Analytics <span className="text-zinc-600">v1.11.0</span></p>
            <ThemeToggle />
          </div>
        ...
        <div className="p-4 border-t border-[#2a2d3a] space-y-2">
          ...
          <p className="text-xs text-zinc-600 text-center">v1.11.0</p>
        </div>
      </aside>
    </>
  );
